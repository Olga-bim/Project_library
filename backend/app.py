from flask import Flask,  send_from_directory, flash, redirect, render_template, request, session, url_for
...
app = Flask(__name__, template_folder='frontend')  # Путь к папке с шаблонами
...
app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{os.path.join(basedir, "models", "library.db")}'
...
# Получаем базовый путь и создаем путь к файлу логов
log_file_path = os.path.join(basedir, 'flask_app.log')  # Лог-файл в той же папке, что и app.py
...
def about_website():
    ...
    app.logger.info('/about_website')
    return render_template('about_website.html')

@app.route('/author_books')
def author_books():
    log_function_time('/author_books')
    app.logger.info('/author_books')
...
def about_website_creator():
    ...
    app.logger.info('/about_website_creator')
...
@app.route('/admin')
def admin():
    log_function_time('admin')  # Логируем вызов функции
    app.logger.info('admin route accessed')  # Логируем доступ к маршруту
    customers = Customer.query.all()
...
def add_customer():
    log_function_time('add_customer')  # Логируем вызов функции
    app.logger.info('Accessing the add_customer route')  # Логируем доступ к маршруту
...
def edit_customer(id):
    log_function_time('edit_customer')
